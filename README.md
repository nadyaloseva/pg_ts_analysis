...
├── pg_python_testing.ipynb # Сравнение результатов с Python-реализацией
```
## Установка
...
Чтобы оценить сглаженное значение в точке $x_0$, строится локальная взвешенная регрессия только по ближайшим соседям, взвешенным по расстоянию. 
